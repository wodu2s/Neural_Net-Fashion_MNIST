...
# 2. 3층 신경망 및 옵티마이저 생성==========
# 입력 크기: 784 (28*28), 은닉층: 100개 뉴런, 출력 크기: 10 (10개 클래스)
...
for i in range(iters_num):
    # 미니배치 획득: 훈련 데이터에서 무작위로 batch_size만큼 선택
    ...
    if i % iter_per_epoch == 0:
        # 훈련 데이터 전체에 대한 정확도 계산
        train_acc = network.accuracy(x_train, t_train)
        # 테스트 데이터 전체에 대한 정확도 계산
        test_acc = network.accuracy(x_test, t_test)
        train_acc_list.append(train_acc)
        test_acc_list.append(test_acc)
        # 현재 에폭, 훈련/테스트 정확도, 손실 값 출력
...
# 손실 곡선 (첫 번째 subplot)
# smooth_curve의 결과 배열 길이가 입력보다 길 수 있으므로 슬라이싱으로 길이를 맞춰 오류를 해결합니다.
